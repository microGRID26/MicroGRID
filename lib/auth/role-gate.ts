...
export async function checkRole(input: RoleCheckInput): Promise<RoleCheckResult> {
  ...
  if (!authUserEmail) {
    return { ok: false, role: null, reason: 'no-email' }
  }
  const { data, error } = await db
    ...
    .maybeSingle()
  if (error) {
    return { ok: false, role: null, reason: 'db-error' }
  }
  if (!data) {
    return { ok: false, role: null, reason: 'no-public-user' }
  }
  if (data.active === false) {
    return { ok: false, role: null, reason: 'inactive' }
  }
  const role = (data.role as string | null) ?? null
  if (!role) {
    return { ok: false, role: null, reason: 'no-role' }
  }
  if (!allowedRoles.includes(role)) {
    return { ok: false, role, reason: 'role-not-allowed' }
  }
