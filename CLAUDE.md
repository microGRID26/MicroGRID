...
- **Planset generator:** `/planset` produces 9 base sheets + 4 enhanced (`?enhanced=1`) = 13 total. RUSH Engineering grade: cover page, project data, site plan with Detail-A, equipment elevation (photo), roof plan with modules, attachment detail (SVG), SLD with callouts/DPC/physical MSP, PCS labels, TAG wire chart, warning labels, placards, conductor schedule, utility battery letter. Seraphim SRP-440-BTD-BG default. Print preloads images + waits for fonts.
