...
- **Planset Phase 6 (Drive auto-pull):** On project load, `/planset` calls `GET /api/planset/drive-photos?projectId=X` which uses `lib/google-drive.ts` (service account JWT via the same `GOOGLE_CALENDAR_CREDENTIALS` env var, `drive.readonly` scope). The route lists children of `07 Site Survey` + `08 Design` subfolders of the project's `folder_id`, filters to image MIMEs, classifies by filename regex (`aerial|satellite|drone|overhead` → PV-1 aerial; `house|front|elevation|property` → PV-1 house; `site.?plan|layout|lot` → PV-3 site plan; `roof.?plan|modules|string|subhub` → PV-4 roof plan; `equipment|inverter|battery|panel|wall` → up to 4 × PV-3.1 equipment slots). Returns proxy URLs pointing at `/api/planset/drive-image/[fileId]` which streams Drive bytes through our server (Drive doesn't support signed URLs). 10 MB size cap + MIME allowlist (`image/{jpeg,png,webp}`) on the proxy. 5-min in-memory cache on listings. 15 s AbortController timeout on all Drive calls. Manual upload still overrides auto-pull — the useEffect fires ONCE per `projectId`, not on image state changes. **Requires:** service account `client_email` granted Viewer on the Shared Drive containing project folders. Without the grant, requests 403 and the page falls back silently to upload placeholders.
