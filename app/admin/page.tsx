...
type Module = 'ahj' | 'utility' | 'hoa' | 'financier' | 'users' | 'crews' | 'sla' | 'info' | 'releases' | 'feedback' | 'audit' | 'permissions' | 'notifications' | 'queue_config' | 'reasons'
...
function FinancierManager({ isSuperAdmin }: { isSuperAdmin: boolean }) {
  const supabase = db()
...
function ReasonsManager({ isSuperAdmin }: { isSuperAdmin: boolean }) {
  const supabase = db()
